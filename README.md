...
这是当前项目的主示例。系统读取用户投诉文本，例如：

```text
外卖送到的时候已经凉了，骑手还态度很差，我要投诉并申请退款。
```

Harness 会把非结构化文本解析为标准 JSON：
...
业务系统可以据此自动分流到物流客服、退款客服或风控客服。自进化的价值在于：如果系统连续把“骑手态度差 + 退款诉求”误判为单纯退款纠纷，Harness 会把 bad case 归因出来，沉淀 Few-shot 或 Skill，后续同类工单会更稳定。

### 场景二：企业内部工单质检

在 IT 运维、人事、财务共享中心等场景中，员工提交的工单描述往往不规范，例如“系统一直闪退，报错 500，今天必须处理”。Harness 可以迁移为：

- 状态：原始工单文本、用户角色、业务系统、历史处理记录。
- 动作：分类、紧急程度、责任团队、是否需要升级。
- 反馈：人工质检结果、SLA 是否超时、是否被退回重派。

当系统频繁误判某类系统错误或紧急程度时，进化器可以生成 Prompt Policy 或 Skill，减少人工重复修正规则。

### 场景三：内容审核与质检标签生成

对于图文、电商评价、客服录音转写等内容，可以用相同框架做标签抽取和质检。比如图像质检任务中，输入不一定是原图，也可以是模型或系统生成的元信息：
...
领域 adapter 将其转换为统一 `State`，Harness 输出是否合规、风险标签和处理建议。如果人工反馈发现“带水印但主体清晰”的图片被过度拦截，系统可以把这个经验写入 Skill，避免后续误杀。

### 场景四：语音客服意图识别

语音场景可先由 ASR 转写为文本，再交给 Harness 做意图识别和紧急度判断。例如：

```text
我的账号突然登不上去了，里面还有余额，麻烦马上帮我处理。
```
...
- 标记高优先级。
- 提取“余额”“无法登录”等关键实体。
- 把错误识别的录音转写样例沉淀为 Few-shot。

这类场景的好处是无需重写主循环，只需要替换 adapter 和 schema。

### 场景五：推荐系统事件流诊断

推荐或搜索系统中，用户行为是连续事件流，例如曝光、点击、停留、跳出、举报。Harness 可以把事件流抽象为状态：

```json
{
  "events": ["show", "skip", "show", "hide", "report"],
  "item_category": "food",
  "session_length": 12
}
```

输出动作可以是“降低同类推荐”“进入负反馈队列”“触发人工复核”。反馈来自后续点击率、投诉率或人工标注。这样就能把原本面向文本工单的自进化闭环迁移到推荐流优化。

### 场景六：比赛或答辩演示

推荐演示路径：

1. 用 `llm-evolve` 跑 100 到 300 条样本，展示真实 LLM 执行、bad case 识别和进化过程。
2. 打开 Dashboard，展示 F1 曲线、Token 消耗、缓存命中和回滚事件。
3. 运行 Transfer Runner，证明同一套 Harness 可迁移到文本、图像元信息、语音转写和推荐事件流。
4. 用 README 中的创新点解释：冻结模型、双层记忆、防退化、Token-Aware 和 SAF 统一抽象。

这一场景主要服务技术比武，能清楚回答“是否满足题目要求”“如何降低 Token”“如何防止局部修复破坏全局能力”“如何迁移到同类任务”。
...
系统支持 `cheap/default/smart` 三档模型配置，用于在成本和能力之间做分工：

| 模型档位 | 当前职责 | 设计目的 |
| --- | --- | --- |
| `cheap` | 批量输出 JSON 解析失败后的低风险格式修复 | 用低成本模型处理语法修复，避免直接拆批增加主模型调用 |
| `default` | 主任务批量结构化抽取、回归验证 | 保持主链路效果稳定 |
| `smart` | Bad Case 根因归因、Prompt/Skill 生成 | 处理复杂推理和策略生成 |

`cheap` 模型只负责“修 JSON 格式”，不负责重新判断业务语义。也就是说，当 `default` 模型输出的批量 JSON 数组不合法时，系统会先让 `cheap` 模型尝试修复括号、包装、尾部解释文本或数组形态；如果修复后仍不满足“对象数量与 batch 数一致”的要求，系统才会拆小批重试。

该设计让多模型分层保持轻量：主执行不被复杂调度影响，复杂归因仍交给 `smart`，低风险格式修复交给 `cheap`，从而在不牺牲稳定性的前提下进一步降低成本。
